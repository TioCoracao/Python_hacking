...
def Key(dados):
    ...
    key = hashlib.sha256(Senha).digest() #Gera Hash que será usada como Key do nosso AES
    aes = pyaes.AESModeOfOperationCTR(key)
    dados_xpx = aes.decrypt(dados) #Criptografa os Dados.
    return dados_xpx
#bloco para decriptografar os arquivos
def xpxActionxpx():
